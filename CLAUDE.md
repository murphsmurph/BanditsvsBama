...
10. **The Bandits live in `OPPONENT`, never in `ROSTER`.** Names and line slots only, from the broadcaster's lines-app screenshots. No sweater numbers or stats were supplied, so `number:null` and nothing statistical is shown. "El Presidente" is Marc Hodges (Bandits president), "Greg" is Greg Mayer, and Joey is listed as "Awesome Joey" at the broadcaster's request. The Alabama packet is Alabama-only. The Bandits have their own tab (BANDITS SHEET → `banditsPages()` in call-sheet.js): ALWAYS 2 Letter-portrait pages regardless of the LAYOUT toggle. Page 1: alphabetical roster rows, blank number box, LAST NAME then first name, ruled notes, plus `OPPONENT.blankRows` empty write-in rows; no positions or line slots on paper (they can change). Page 2: blank lines grid (LW/C/RW, LD/RD/G), PP1–PK2 and goalie write-ins, ruled notes. Printing from that tab prints only that packet (print CSS shows the active `.view`). They also appear in Broadcast Mode.
